...
class EtlAnuncio(Etl.EtlFacebookAbs):
    ...
    def limpieza_de_anuncios(self):
        ...
        dao_extrac_prop = adminDao.AdministradorDao(DaoProp.DaoExtraccionProp, txt_anuncios, 'anuncio').dao
        ...
        return self.sql_context.sql("select " + str(dict_prop_anuncios['atributos']) + ' from anuncios')

    @Log.logger('Extraccion')
    def extrae(self):
        dao_anuncio = adminDao.AdministradorDao(daoCon.DAOAnuncio, self.dto_credenciales).dao
        self.anuncios = dao_anuncio.obten()
        self.anuncios['data_date_part'] = Util.obt_fecha_actual()

    @Log.logger('Transformacion')
    def transforma(self):
        self.detalle_de_anuncios = self.limpieza_de_anuncios()
        self.detalle_de_anuncios.coalesce(1).write.format("com.databricks.spark.csv").save('anuncios_24_dic.csv', header='true')
    ...
    def carga(self):
        ...
        # Guarda a oracle
        self.detalle_de_anuncios.write.jdbc(url=url_jdbc, table="FB_DETALLE_ANUNCIOS", mode='append', properties=prop_conx)
        # Guarda en object storage csv
        nom_archivo = 'detalle_de_anuncio_' + Util.obt_cadena_fecha_actual() + '.csv'
        nom_ruta_hdfs = 'oci://bdcsce@axk8tyxiw9wz/facebook/marketing/anuncio/' + nom_archivo

        self.detalle_de_anuncios.coalesce(1).write.format('com.databricks.spark.csv').save(nom_ruta_hdfs, header='true')
